...
/** Zitadel: OpenID configuration reachability */
export function zitadelProbe(): Dependency {
  return {
    name: "zitadel",
    tier: 1,
    cacheTtlMs: 30_000,
...
/** S3: endpoint reachability probe */
export function s3Probe(): Dependency {
  return {
    name: "s3",
    tier: 1,
    cacheTtlMs: 30_000,
    ...
    probe: async () => {
      ...
      if (!endpoint) {
        // If endpoint is not explicitly configured, rely on provider defaults.
        // This still validates that required env is present for runtime usage.
        return true;
      }
