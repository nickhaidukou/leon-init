...
import { createStorageAdminClient } from "@api/services/storage-client";
...
export const insightsRouter = createTRPCRouter({
  ...
  audioUrl: protectedProcedure
    ...
    .query(async ({ ctx: { db, teamId }, input }) => {
      ...
      const storageClient = await createStorageAdminClient();
      ...
      if (!audioPath) {
        ...
        try {
          const result = await generateInsightAudio(storageClient, insight);
