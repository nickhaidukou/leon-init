...
mock.module("@midday/storage", () => ({
  createSignedUrl: mock(async () => "https://example.com/signed-url"),
  createPresignedUploadUrl: mock(
    async () => "https://example.com/presigned-upload",
  ),
  createPublicUrl: mock(() => "https://example.com/public-url"),
  remove: mock(async () => ({ bucket: "vault", key: "test-path" })),
  download: mock(
    async () =>
      ({
        data: new Uint8Array(),
...
// Mock @api/services/storage-client
mock.module("@api/services/storage-client", () => ({
  createStorageClient: mock(async () => ({})),
  createStorageAdminClient: mock(async () => ({})),
}));
