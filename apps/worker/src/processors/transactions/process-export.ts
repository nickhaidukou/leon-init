...
import { downloadVaultFile } from "../../utils/storage";
          ...
          batch.flatMap((transaction) => {
            ...
            return (transaction.attachments ?? []).map(
              async (attachment, idx2: number) => {
                ...
                const attachmentPath = (attachment.path ?? []).join("/");

                let data: Blob | undefined;
                try {
                  data = await downloadVaultFile(attachmentPath);
                } catch (error) {
                  this.logger.warn("Failed to download attachment for export", {
                    attachmentPath,
