...
import { createVaultSignedUrl, uploadVaultFile } from "../../utils/storage";
...
export class WhatsAppUploadProcessor extends BaseProcessor<WhatsAppUploadPayload> {
  async process(job: Job<WhatsAppUploadPayload>): Promise<void> {
    ...
    try {
      ...
      try {
        uploadData = await uploadVaultFile({
          key: filePathStr,
      ...
      try {
        signedUrl = await createVaultSignedUrl({
          key: pathForSignedUrl,
          expireIn: 1800,
        });
      } catch (error) {
        throw new Error(
